...
- Gesprochene Befehle laufen **ausschließlich** durch `VoiceListen.handle_text()` — ein Parser, zwei Eingänge (Bot-B-Bridge und später eigenes Zuhören). Ausgeführt wird über eine synthetische Message, nie über `ctx.invoke`, damit alle Checks mitlaufen; `ctx.author` muss ein echtes `Member` sein (nur daran hängt `.voice`) → [ADR 0011](docs/adr/0011-sprachsteuerung-ein-parser-zwei-eingaenge.md).
